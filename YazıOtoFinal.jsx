...
function combineShortSentences(sentences, minChars, maxChars) {
    ...
    for (var i = 0; i < sentences.length; i++) {
        ...
        if (currentSentence === null) {
            ...
            // 1. Şu anki uzunluk minimumun altındaysa VEYA
            // 2. Birleşince maksimumu aşmıyorsa VE zaman farkı küçükse
            var shouldCombine = (currentSentence.length < minChars || 
                               (potentialCombinedLength <= maxChars && timeDiff < 0.5));
...
function createTextDisplayEffect() {
    ...
    try {
        ...
        if (settings.minChars > 0 || settings.maxChars > 0) {
            ...
            alert("Cümleler " + settings.minChars + "-" + settings.maxChars + " karakter aralığına göre birleştirildi. Yeni cümle sayısı: " + sentences.length);
        }
